...
$pagePath = Join-Path $PSScriptRoot 'Calc1_The_A_Plan.html'
$deckPath = Join-Path $PSScriptRoot 'MAT191_Calculus1_Masterclass_Deck.html'
$keyPath = Join-Path $PSScriptRoot '.for_yashi_key'
$url = 'http://127.0.0.1:4174/Calc1_The_A_Plan.html'
$model = 'deepseek/deepseek-v4-flash-0731'
$listener = [System.Net.Sockets.TcpListener]::new([System.Net.IPAddress]::Loopback, 4174)

$systemPrompt = @'
You are a Calculus and Analytic Geometry 1 coach for a community-college student whose goal is an A this semester.

Write like a calm tutor. Short sentences. Direct. No slogans, no emojis, no em dashes, no "unlock your potential," no "delve," no "moreover."

Teach formulas as English sentences first, then symbols. Show the algebra that actually loses points. Difficulty should match a standard Calc 1 exam, not a contest.

Follow the requested JSON shape exactly. Return only valid JSON with no markdown fences or extra commentary.
'@
...
try {
  while (((Get-Date) - $lastRequest).TotalHours -lt 2) {
    ...
    try {
      ...
      if ($method -eq 'GET' -and ($path -eq '/' -or $path -eq '/Calc1_The_A_Plan.html' -or $path -eq '/Calc1_A_Plan.html')) {
      ...
      if ($method -eq 'GET' -and $path -eq '/MAT191_Calculus1_Masterclass_Deck.html') {
        if (Test-Path -LiteralPath $deckPath) {
          $body = [System.IO.File]::ReadAllBytes($deckPath)
          Write-Response -Stream $stream -Status 200 -Reason 'OK' -ContentType 'text/html; charset=utf-8' -Body $body
          continue
        }
      }
